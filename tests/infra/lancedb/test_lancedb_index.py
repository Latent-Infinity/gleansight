...
def test_lancedb_query_nonexistent_table_raises(tmp_path: Path) -> None:
    lancedb = pytest.importorskip("lancedb")
    has_connect = (
        hasattr(lancedb, "connect")
        or hasattr(lancedb, "lancedb_connect")
    ...
    if not has_connect:
        pytest.skip("lancedb connect API not available")
    index = LanceDBVectorIndex(LanceDBConfig(path=tmp_path))
    # Querying a nonexistent table should raise an exception
    with pytest.raises(Exception):
        index.query([0.1, 0.2, 0.3], limit=1)
