...
class PdfDownloader:
    ...
    source_rate_limits: dict[str, float] = field(default_factory=dict)
    rate_limiter: RateLimiter = field(init=False)
    _source_rate_limiters: dict[str, RateLimiter] = field(init=False)

    def __post_init__(self) -> None:
        self.rate_limiter = RateLimiter(self.rate_limit_per_second)
        self._source_rate_limiters = {
            source: RateLimiter(rate) for source, rate in self.source_rate_limits.items()
        }

    def _get_rate_limiter(self, source: str) -> RateLimiter:
        """Get the rate limiter for the given source, falling back to default."""
        if source and source in self._source_rate_limiters:
            return self._source_rate_limiters[source]
    ...
    def download(self, url: str, dest_path: Path, *, source: str = "") -> None:
        ...
        for attempt in range(self.max_retries):
            try:
                self._get_rate_limiter(source).acquire()
                ...
                logger.warning(
                    "Timeout downloading %s (attempt %d/%d)", url, attempt + 1, self.max_retries
                )
                ...
                if status_code == 429:
                    ...
                    logger.warning(
                        "Rate limited downloading %s (attempt %d/%d)",
                        url,
                        attempt + 1,
                        self.max_retries,
                    )
                    ...
                    last_error_code = ErrorCode.NETWORK_ERROR
                    logger.warning(
                        "Server error %d downloading %s (attempt %d/%d)",
                        status_code,
                        url,
                        attempt + 1,
                        self.max_retries,
    ...
    def _do_download(self, url: str, dest_path: Path) -> None:
        ...
        headers = {
            "User-Agent": "Mozilla/5.0 (compatible; Gleansight/1.0; academic research tool)",
            ...
            with client.stream("GET", url) as response:
                ...
                with dest_path.open("wb") as f:
                    for chunk in response.iter_bytes():
                        f.write(chunk)
        self._validate_pdf(dest_path)

    @staticmethod
    def _validate_pdf(path: Path) -> None:
        """Check that the downloaded file starts with the PDF magic number."""
