...
| threshold `τ` | **0.45** (`approved_default_tunable`) | packet 2b plus explicit human activation 2026-08-29: compute the discrete novelty term below, then zero it when `evidence < 0.45`. Override via `settings.nsqd.novelty_threshold_tau`; the CLI has no `--tau` switch. The value is active but not frozen. |
...
`axis-candidate-contract.yaml` and `validate_operator_f_axis_proposal` define a fail-closed, report-only proposal boundary. One proposal must bind a non-registered measurable axis, source and snapshot digests, current-axis and shuffled-axis controls, required metric declarations, and detached trust whose approval binds the canonical proposal digest. Post-approval mutation is rejected. The preserved F-PROP-001 record states `human_approved` for `evaluation_only`, but no detached trusted approval tuple is present, so current trust classifies it as `historical_claim_unverified` and runtime validation rejects it without deciding whether an actual human decision occurred. Its deterministic pilot remains descriptive: it compares current axes, current plus the candidate, and current plus a shuffled control over the three records with registered coordinates while separately accounting for all five observed records. The predeclared five-fold source-group protocol cannot execute with three eligible source groups. The current independently technically reviewed implementation binding is `docs/reviews/nsqd-operator-f-readiness-2026-09-12-implementation-binding/` (`e82490162af6f8da4507aef5cfe019869a2caecdaa3cf985a0751e794c8d62fe`), referenced by the reviewed activation pointer sync at `docs/reviews/nsqd-operator-activation-2026-09-12-pointer-sync/` (`c767f3ced5be85ddd1aa8432106e207e6b0c262593a90a69a910a4518444ebed`). Held-out gain and stability, redundancy, residual-variation, quality, confound evidence, and numeric thresholds remain missing, so evidence sufficiency and schema admission remain false. Technical review grants no human, evidence, schema, or runtime authority. Validation or evaluation does not register the axis, mutate the archive, write corpus facts, or authorize Operator F.

### ALG-OP-G — Failure-resurrection record contract (deferred)

`failure-record-contract.yaml`, the v2 contract successor, and `validate_operator_g_failure_record` define a fail-closed, report-only evidence boundary. A record must identify a registered experiment artifact as its source class and bind an immutable source artifact, measured failure evidence, original conditions, one allowed resurrection scope, changed-condition triggers, restart conditions, and a distinct independent human review. Test failures, job errors, sufficiency failures, synthetic or inferred failures, source-class metadata, and absence-of-success claims are rejected or ignored as non-records. The current census at `docs/reviews/nsqd-operator-g-readiness-census-2026-09-12-schema-closure/` (`9af4910deb08b980a6a8ee0feda263825cada46703b13d54e00aeb983ea06680`) is complete over 354 structured files (`scope_snapshot_digest=b930b81be31ebdad3e9007c6b96963939dc73915da6a896f8914ba9e3b2abbb4`) with 49 source bindings, zero trusted approvals, zero trusted evidence artifacts, and zero qualifying records. It chains directly from immutable packet `0649829a742d6433ec757afb5620cfa5f03665372d08196b47259a432bbb3f49`; earlier censuses remain historical. `authorization_state=report_only`, `operator_g_eligible=false`, `evidence_sufficient=false`, and all authority flags are false. The current G packet has no self-authored technical review. Validation or digesting does not write corpus facts, restart work, or authorize Operator G.
...
| `ALG.NOV.TAU` | 120 balanced accepted autonomous labels over persisted measurements | overall novel false-kill ≤5% and each policy ≤10%; choose highest admissible staged edge | 30 near-duplicate + 30 novel per policy | `0.45`, `approved_default_tunable` | `docs/ablations/alg-novelty-tau.md` and packet 2b (activated 2026-08-29; **not frozen**) |
| `ALG.FREEZE` | existing ALG.* probes | freeze-now vs keep-tunable | all families | keep tunable | `docs/ablations/alg-freeze.md` (packet 3 2026-08-26; **not frozen**) |

Human review validated the five ALG.* geometry/status/viability artifacts and the acquisition-budget math/state probe but did not approve a numeric/default freeze. The acquisition-budget outcome is `approved_default_tunable`: retain 3 / 25 / 3 / 2 for compatibility, not as a demonstrated optimum. Keep every reviewed knob tunable. Packet 1a records v1 “24 months” as **730 days**, overridable via `window_days`; 12/36 day-length sensitivity is filed and not frozen. Packet 2a established report-only novelty-threshold semantics; packet 2b plus the separate human decision subsequently activated **`τ = 0.45` as `approved_default_tunable`**. Packet 3's no-freeze decision remains in force (`docs/ablations/alg-freeze.md`).
