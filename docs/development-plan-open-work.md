...
**Outstanding Blockers / Human Decisions**: V0A approved DATA-01a/b/c. **This closeout plan (V0–V3, V6–V7) is complete.** Product decisions V0.4/0.5/0.6 and V0A.5 (three papers) remain recorded below. Gitignored `data/db` / `data/blobs/md` are still not the approved fixture set. NSQD N1–N10 including N6 injected acquisition orchestration and durable digest approval are done; default production paper-pipeline composition remains pending. DATA-NSQD-04 is acquired. Approved DATA-NSQD-03 remains pending (`docs/development-plan-ns-qd.md`).
