...
**Outstanding Blockers / Human Decisions**: V0A approved DATA-01a/b/c. **This closeout plan (V0–V3, V6–V7) is complete.** Product decisions V0.4/0.5/0.6 and V0A.5 (three papers) remain recorded below. Gitignored `data/db` / `data/blobs/md` are still not the approved fixture set. NSQD N1–N10 including N6 default paper-runtime composition are done. ALG.K is filed as a synthetic math/state probe; the other ALG.* ablations are LLM-scored artifacts whose prompts and results require human validation; defaults are not frozen. DATA-NSQD-04 is acquired. DATA-NSQD-03 is approved and its `finance/1 production_valid` path is verified (`docs/development-plan-ns-qd.md`).
