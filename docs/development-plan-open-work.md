...
**Outstanding Blockers / Human Decisions**: V0A approved DATA-01a/b/c. **This closeout plan (V0–V3, V6–V7) is complete.** Product decisions V0.4/0.5/0.6 and V0A.5 (three papers) remain recorded below. Gitignored `data/db` / `data/blobs/md` are still not the approved fixture set. NSQD N1–N10 including N6 default paper-runtime composition are done. Analyze CLI defaults to Qwen3.6 chat. Human validation of ALG.K and the four LLM-scored ALG.* prompts/results completed 2026-08-24; the current defaults are retained but explicitly remain tunable and not frozen. Human review approved ALG.ACQUISITION_BUDGET on 2026-08-25 as `approved_default_tunable`: keep 3 batches/pass / 25 candidates/call / 3 imports/pass / 2 approved rechecks for compatibility, not as an optimum or freeze. Operator B is supported, non-default, and composition-gated via `settings.nsqd.enabled_operators` (default `A` only); `diverge --operator` may request A/B but cannot widen the allowlist. Packet 2b contains 120 policy-balanced autonomous-agent labels and recommended `τ = 0.45`; a separate explicit human decision activated it as `approved_default_tunable`, not frozen. Operator E is experimental, executable, and off by default through the same composition allowlist; C, D, F, and G remain runtime-deferred. After the report-only calendar replay, the human retained fixed 730-day runtime semantics and rejected calendar-month subtraction for runtime on 2026-09-03. DATA-NSQD-04 is acquired. DATA-NSQD-03 is approved and its `finance/1 production_valid` path is verified (`docs/development-plan-ns-qd.md`).
