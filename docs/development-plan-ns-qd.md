...
**Supersedes**: `docs/development-plan-ns-qd.md` v1.6.61 wording (same file, revision)
...
| Operators C, D, F, and G | C/D deferred; D/F/G collection contracts executable but non-authorizing | N/A (deferred) | — | C needs evidence; D remains blocked on C; F/G need valid records and ablations |
...
| NSQD.OPERATOR.ACTIVATION_PLAN.v1 | CLI selection is A/B and cannot widen composition. C's second provenance-bound evidence cycle rejects its preferred path for semantic-object mismatch and its backup for direct prior art; candidate outputs remain empty. D/F/G have executable fail-closed report-only data contracts; D remains blocked on C, and F/G have no qualifying evidence. C, D, F, and G stay runtime-disabled. E is experimental, executable, and off by default (`operator-e-atypical-combination/1`); only explicit configuration may add E; rarity-only generation is rejected; report-only JEPA artifacts remain ungenerated | Optional packet 5 | Data Contract, Behavior | LOCAL-NSQD-D | EV-N20 |
...
| EV-N20 | NSQD.STATUS.CALENDAR_REPORT.v1, NSQD.OPERATOR.ACTIVATION_PLAN.v1 | `uv run pytest tests/nsqd/test_operator_activation_packets.py tests/nsqd/test_operator_c.py tests/nsqd/test_operator_c_evidence_packet.py tests/nsqd/test_operator_c_evidence_cycle_2.py tests/nsqd/test_operator_d_contract.py tests/nsqd/test_operator_e.py tests/nsqd/test_operator_e_cooccurrence.py tests/nsqd/test_operator_e_report_only_candidates.py tests/nsqd/test_operator_e_broader_prior_art.py tests/nsqd/test_operator_e_runtime.py tests/nsqd/test_operator_f_contract.py tests/nsqd/test_operator_g_contract.py tests/nsqd/test_operator_baselines.py tests/nsqd/test_status_window_ablation.py tests/nsqd/test_status_window_receipt_replay.py tests/nsqd/test_map.py tests/nsqd/test_cli.py tests/nsqd/test_operator_a.py tests/nsqd/test_operator_b.py -q --no-cov && uv run python scripts/replay_status_window_ablation.py --verify-current-receipt && uv run python scripts/replay_status_window_ablation.py --output-dir docs/reviews/nsqd-status-window-calendar-replay-2026-09-02` | Optional packets 1c/5 | Required |
