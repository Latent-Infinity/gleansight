...
**Outstanding Blockers**: None for Operator A, supported non-default Operator B, N11.4 threshold activation, the status-window decision, or experimental Operator E. Default composition is `A` via `settings.nsqd.enabled_operators`; configuration may explicitly add B or E. `diverge --operator` remains A/B-only and cannot widen authorization. After the approved-snapshot portable replay, the human retained 730-day status-window semantics and rejected calendar-month subtraction for runtime. E is experimental, executable, and off by default (`operator-e-atypical-combination/1`). C, D, F, and G remain runtime-deferred.
...
| 1.6.38 | 2026-09-01 | Operator E attaches the JEPA finance same-policy co-occurrence snapshot as corpus rarity only; combinations stay empty and A/B baselines remain unexecuted. Four-command gate: 1357 passed, 2 skipped, 92.20%; dedicated NSQD: 685 passed, 92.59%. |
| 1.6.39 | 2026-09-02 | JEPA/E accounting now distinguishes the executed structural screen from unexecuted matched-count Operator A/B baselines. Reviewed usefulness remains unmeasured pending independent human review, and Operator E remains unauthorized. The co-occurrence binder requires exact packet-declared provenance, records, and evidence. Four-command gate: 1358 passed, 2 skipped, 92.17%; dedicated NSQD: 686 passed, 92.53%. |
| 1.6.40 | 2026-09-02 | JEPA records three Operator A inversions and three Operator B whitespace sketches at matched count k=3 as unreviewed report-only baselines. Usefulness remains unmeasured; Operator E stays unauthorized. Four-command gate: 1362 passed, 2 skipped, 92.20%; dedicated NSQD: 690 passed, 92.56%. |
| 1.6.41 | 2026-09-02 | Replace the sketch-only matched-count baseline with a real report-only evidence pipeline: three Operator A and three Operator B Diverge → Ground → Score artifacts bind to approved finance snapshot `bb63826c…e8a5` / corpus version 11 with real `qwen3-embedding:latest` provenance; three report-only Operator E artifacts, a blinded 9-item human usefulness packet, and a separate audit manifest are sealed with all human scores null and runtime disabled. Four-command gate: 1422 passed, 2 skipped, 92.01%; dedicated NSQD: 750 passed, 92.20%. |
| 1.6.42 | 2026-09-02 | Audit-remediate the JEPA A/B/E report-only packet: remove fabricated baseline dval, rerun all six A/B cards on the preserved scratch approved snapshot, bind the full Ollama manifest digest and execution receipt, strengthen validator checks against runtime SQLite/LanceDB state and recomputed grounding/score contracts, and separate prior-report review from the new pending execution-bundle review. Four-command gate: 1434 passed, 2 skipped, 92.08%; dedicated NSQD: 762 passed, 92.31%. |
| 1.6.43 | 2026-09-02 | Remove the fresh-clone portability blocker from the JEPA A/B/E packet: ordinary validation no longer opens historical scratch paths, receipt-to-packet binding is durable and hermetic, optional local scratch verification is explicit, and `scripts/replay_jepa_operator_baselines.py` reruns or verifies the real Ollama scratch evidence without touching production state. Four-command gate: 1446 passed, 2 skipped, 92.13%; dedicated NSQD: 774 passed, 92.40%. |
| 1.6.44 | 2026-09-02 | Close final JEPA/NSQD code-quality and security blockers without changing report-only semantics: blinded review content is now recomputed from artifact-owned canonical fields, snapshot provenance is described truthfully as approved corpus snapshot `bb63826c…e8a5` with finance/1 filtering, scratch/history path guards reject symlink and confinement abuse, approved manifest/projection/excerpt reads reject symlink aliases, and packet/audit digests were resealed consistently. Full gate: 1454 passed, 2 skipped, 92.16%; focused JEPA packet/validator/co-occurrence suite: 105 passed; touched `operator_baselines.py`: 92%; live replay 3A/3B; historical receipt 6/6. |
| 1.6.45 | 2026-09-02 | Close the residual JEPA/NSQD medium/minor/context findings: fresh replay scratch dirs are restricted to direct children of existing canonical temp roots, SQLite receipt digests stream under a byte cap, replay uses shared public trusted-file helpers, Operator E binds canonical atypicality interpretation plus explicit `nearest_prior_combinations`, and plan/context commands were aligned to the current report-only human-pending packet. Full gate: 1460 passed, 2 skipped, 92.19%; focused JEPA packet/validator/co-occurrence suite: 111 passed; dedicated NSQD: 788 passed, 92.49%; touched `operator_baselines.py`: 92%; touched `trusted_files.py`: 97%; live replay 3A/3B; historical receipt 6/6. |
| 1.6.46 | 2026-09-02 | Fix intermittent fresh logical replay drift from small Ollama-derived numerics without weakening exact structure or historical receipts: bounded study over the preserved failed scratch plus five sequential fresh replays found max finite drift ≈2.03e-4, so fresh logical replay now uses recursive finite-float tolerance `1e-3` while keeping ids/hashes/order/non-float fields exact and leaving `--verify-current-receipt` fully byte/hash exact. Full gate: 1461 passed, 2 skipped, 92.19%; focused JEPA packet/validator/co-occurrence suite: 112 passed; dedicated NSQD: 789 passed, 92.49%; touched `operator_baselines.py`: 92%; touched `trusted_files.py`: 97%; live replay 3A/3B repeated 5x; historical receipt 6/6. |
| 1.6.47 | 2026-09-02 | Fix the remaining fresh logical replay failure where permitted float drift changed `measurement_artifact_digest`: fresh replay now applies the derived-digest exemption only when both sides are canonical tau-measurement mappings containing every `TAU_MEASUREMENT_ARTIFACT_FIELDS` entry, validating each side's digest against its own canonical float-bearing measurement mapping before omitting only that derived field from cross-side comparison; arbitrary mappings with the same key name still compare exactly, while all underlying measurement fields, ids, hashes, order, and historical receipt hashes remain exact. Full gate: 1463 passed, 2 skipped, 92.19%; focused JEPA packet/validator/co-occurrence suite: 114 passed; dedicated NSQD: 791 passed, 94.64%; touched `operator_baselines.py`: 92%; live replay 3A/3B repeated 5x; historical receipt 6/6. |
| 1.6.48 | 2026-09-02 | Record the completed operator-label-blinded human usefulness review without mutating A/B/E candidate artifacts: the packet and audit manifest now carry the exact human reviewer label, UTC timestamp, duplicate family, descriptive raw plus duplicate-collapsed metrics, and explicit abstention-safe aggregation semantics; validators recompute the completed review state and preserve report-only/runtime-disabled boundaries. Full gate: 1482 passed, 2 skipped, 92.00%; focused JEPA packet/validator/co-occurrence suite: 133 passed; dedicated NSQD: 810 passed, 94.38%; touched `operator_baselines.py`: 90.38%; live replay 3A/3B; historical receipt 6/6. |
| 1.6.49 | 2026-09-02 | Bind the sealed JEPA report-only Operator E candidate artifacts onto the Operator E evidence plan without treating them as generated combinations or DivergeUseCase outputs: the plan stays `evidence_plan` / `algorithm_identity=not_run`, completed descriptive usefulness review and higher Operator A scores remain non-authorizing, and runtime stays disabled. Four-command gate: 1490 passed, 2 skipped, 92.11%; focused JEPA/E packet suite: 149 passed; dedicated NSQD plus NSQD fact tests: 731 passed, 91.53% on `src/nsqd`; touched `operator_e.py`: 98%. |
| 1.6.50 | 2026-09-03 | Seal and harden the report-only status-window calendar replay artifact from the verified historical scratch receipt: approved snapshot `bb63826c…e8a5` / corpus version 11 yields 11 receipt-bound rows with real persisted `harvested_at=2026-09-02T06:45:00+00:00`; the validator is fail-closed on exact extracted-row digest/order/counts, exact `sealed_at_utc`, required evidence fields, and recomputed scenario outputs; receipt input is fixed to the approved repo packet through bounded trusted reads, scratch `nsqd.sqlite` / `index` leaf symlinks are rejected, SQLite verification is read-only, and output paths are confined to the sealed repo directory or non-symlink allowlisted temp roots. The portable replay still proves an explicit zero-delta current-as-of case and a derived boundary sensitivity where lifecycle semantics diverge (`730 days` stale vs `24 calendar months` current) while cell-status outputs remain unchanged. Runtime `STATUS_WINDOW_DAYS` stays 730 and calendar activation remains pending a separate human decision. Full gate: 1525 passed, 2 skipped, 92.14%; focused security/calendar suite: 308 passed; dedicated NSQD: 853 passed, 94.51%; touched `status_window_replay.py`: 94%; touched `trusted_files.py`: 95%. |
| 1.6.51 | 2026-09-03 | Seal a broader report-only Operator E prior-art evidence packet for the three immutable JEPA E-report artifacts without authorizing E or turning them into generated combinations: the new digest-bound packet binds exact source candidate bytes (`operator-e-report-only-candidates.json` SHA `c2fee6a3…3783`), its canonical semantic content, exact artifact hashes, thirteen verified primary records, bounded overlap conclusions, and absence-is-not-proof language; named candidate/results sources now bind their own file SHA separately from the enclosing review-summary digest, and the broader packet rejects resealed query/evidence drift. `operator-e.yaml` keeps `algorithm_identity=not_run`, `candidate_combinations=[]`, `candidate_outputs=[]`, `evidence_sufficient=false`, and `runtime_authorized=false`. Exact EV-N20 command: 343 passed; exact EV-N21 command: 45 passed; full gate: 1560 passed, 2 skipped, 91.91%; dedicated NSQD: 888 passed, 92.00%; touched `operator_e.py`: 91%. |
| 1.6.52 | 2026-09-03 | Record separate downstream decisions without falsely claiming nonexistent runtime: the broader Operator E packet passed independent five-way technical review, the human approved evidence sufficiency for experimental off-by-default implementation and authorized that experimental runtime path, but authorization remains ineffective while `algorithm_identity=not_run` and generated outputs remain empty. Separately, the human retained the existing 730-day window and rejected 24-calendar-month subtraction for runtime. Verification remains EV-N20: 343 passed; EV-N21: 45 passed; full gate: 1560 passed, 2 skipped, 91.91%; dedicated NSQD: 888 passed, 92.00%. |
| 1.6.53 | 2026-09-03 | Implement Operator E as an experimental off-by-default runtime capability: composition may add E, default remains A-only, CLI stays A/B-only, rarity-only generation is rejected, same-policy and cross-policy tracks stay unpooled, and report-only JEPA artifacts remain ungenerated. Four-command gate: 1571 passed, 2 skipped, 91.98%; EV-N20 pytest: 354 passed; dedicated NSQD plus NSQD fact tests: 812 passed, 91.38% on `src/nsqd`; touched `operator_e.py`: 92%. |
| 1.6.54 | 2026-09-04 | Final review preserves Operator E as an experimental off-by-default capability and the CLI as A/B-only, while fixing trusted-file parent traversal outside approved roots. Configuration may explicitly add E with target-bound proof; report-only JEPA artifacts remain ungenerated. Full gate: 1572 passed, 2 skipped, 91.97%; EV-N20 pytest: 355 passed; EV-N21: 45 passed; dedicated NSQD plus NSQD fact tests: 900 passed, 92.10% on `src/nsqd`. |
| 1.6.55 | 2026-09-04 | Close the allowlisted Operator E score/rescore provenance contract: Diverge → Ground → Score and later rescore preserve `generating_operator=E`, default composition still rejects E, scored/archive cards accept only runtime operators A/B/E, and the CLI remains A/B-only. Four-command gate: 1574 passed, 2 skipped, 91.97%; touched `card.py`: 100%. |
| 1.6.56 | 2026-09-05 | Seal a second report-only Operator C evidence cycle and make the Operator F/G collection boundaries executable without activating them: C rejects the preferred path for semantic-object mismatch and the backup for direct prior art, keeps zero candidate outputs, and remains evidence-insufficient and runtime-unauthorized; F/G validators enforce digest-bound non-authorizing contracts; the default E composition rejects E while the reviewed override explicitly enables it, and the CLI remains A/B-only. Full gate: 1619 passed, 2 skipped, 92.07%; EV-N20: 400 passed; focused F/G contract coverage: 95.31%. |
...
| 1.6.64 | 2026-09-06 | Strengthen the C/D authority regressions after skeptical review: executable metadata is now asserted exactly for C's separate evidence, acceptance, and activation gates and D's typed-graph plus accepted-bridge prerequisites; documentary tests bind the complete C/D decision rows and typed-graph gate. Four-command gate: 1638 passed, 2 skipped, 92.09%. |
| 1.6.65 | 2026-09-07 | Bind the existing JEPA `validation_target` hypothesis as report-only Operator F proposal F-PROP-001, approve it only for evaluation, and execute a deterministic current/candidate/shuffled descriptive pilot. All five labels are observed, only three records are coordinate-eligible, every track occupies three cells at density 1.0, and held-out gain remains unmeasured. `schema_admission_recommended`, `evidence_sufficient`, schema mutation, and runtime authorization remain false; C, D, and G stay deferred. |
| 1.6.66 | 2026-09-07 | Seal the third report-only Operator C evidence cycle after independent review approved only its negative and insufficient-evidence conclusion. The receipt-bound packet preserves zero candidate outputs, evidence insufficiency, no human acceptance, no Operator C/D authorization, and blocked Operator D status. |
| 1.6.67 | 2026-09-08 | Reconcile current operator authority to the digest-bound C full-text follow-up and F/G readiness inventories. Browser acquisition and historical OpenReview v2 full-text inspection are attested, not offline-replayable: source and bibliographic response bytes were removed, so verification covers packet, receipt-metadata, extract, and detached-chain integrity but cannot replay quote containment or bibliographic responses. Bounded checks recorded zero interactions within their snapshots without proving universal noninteraction. C has no accepted typed bridge and C/D remain blocked. F remains non-admitting with five observed and three eligible records, an unexecutable predeclared five-fold source-group protocol, missing held-out/stability/redundancy/residual/quality/confound evidence, and no human thresholds. G has zero qualifying approved records. Packet digests: C `7dc27965…f43`, F `939e528b…9d8`, G `96e13bb1…ee2`. Four-command gate: 1899 passed, 2 skipped, 92.20%; EV-N20 pytest: 680 passed; dedicated NSQD: 1227 passed, 92.43%. |
...
| Operators C, D, F, and G | C/D deferred; D/F/G collection contracts executable but non-authorizing | N/A (deferred) | — | C has no accepted typed bridge after the bounded full-text follow-up; D remains blocked on C; F has 5 observed/3 eligible and no executable held-out protocol or thresholds; G has zero qualifying approved records |
...
| NSQD.SURFACE.UNIFIED.v1 | `gleansight` exposes harvest, map, diverge, ground, gate, archive, project, acquire, rescore, tau, and skeleton without breaking `papers`; the desktop app keeps evidence screens and adds the matching NSQD screens. Diverge remains A/B-only | N10 | Behavior | LOCAL-NSQD-U | EV-N18 |
...
| NSQD.STATUS.CALENDAR_REPORT.v1 | Packet 1c defines a report-only 730-day versus 24-calendar-month UTC comparison with invalid-day month-end clamping; after the sealed portable replay over approved snapshot `bb63826c…e8a5`, the human retained the overridable 730-day runtime window and rejected calendar-month subtraction for runtime | Optional packet 1c | Data Contract | LOCAL-NSQD-M | EV-N20 |
| NSQD.OPERATOR.ACTIVATION_PLAN.v1 | CLI selection is A/B and cannot widen composition. The current C integrity successor is `docs/reviews/nsqd-operator-c-evidence-resolution-2026-09-11-readme-correction/`; its preserved evidence remains negative and insufficient, no human acceptance was requested, and C/D remain blocked. F-PROP-001 records `human_approved` for `evaluation_only`, but current trust classifies it as `historical_claim_unverified` because no detached trusted approval tuple is present. The current reviewed F binding is `docs/reviews/nsqd-operator-f-readiness-2026-09-12-implementation-binding/`, and the current reviewed activation overlay is `docs/reviews/nsqd-operator-activation-2026-09-12-pointer-sync/`; neither grants authority. The current G census is `docs/reviews/nsqd-operator-g-readiness-census-2026-09-12-schema-closure/` (`9af4910deb08b980a6a8ee0feda263825cada46703b13d54e00aeb983ea06680`), chained from packet `0649829a742d6433ec757afb5620cfa5f03665372d08196b47259a432bbb3f49`, complete over 354 structured files with 49 source bindings, zero trusted approvals, zero trusted evidence artifacts, and zero qualifying records. C, D, F, and G stay runtime-disabled. E is experimental, executable, and off by default (`operator-e-atypical-combination/1`); only explicit configuration may add E, and report-only JEPA artifacts remain ungenerated | Optional packet 5 | Data Contract, Behavior | LOCAL-NSQD-D | EV-N20 |
| NSQD.JEPA.IDEAS_GAPS_REPORT.v1 | Given the five approved finance projections, the bounded report distinguishes one direct JEPA paper from four adjacent papers and emits ten source-bound facts, four inferred corpus gaps, three report-only testable ideas, bounded prior-art caveats, and one non-admitted axis hypothesis on the approved corpus snapshot with finance/1 filtering. It does not generate Operator E combinations from the report, records a completed descriptive blinded usefulness review, binds a broader thirteen-source report-only prior-art packet for the three E artifacts, writes no corpus facts, and does not turn report-only E artifacts into generated combinations | JEPA finance report | Data Contract | LOCAL-NSQD-D | EV-N21 |

**Not activated on smoke:** any “production novelty term > 0” fact; any “smoke card became a production elite” fact. **Non-default:** Operator B is supported and composition-gated. **Experimental, non-default:** Operator E is executable and composition-gated. **Deferred:** operators C, D, F, and G.
...
1. **Status recency window:** packet 1a accepted — v1 “24 months” is **730 days**, overridable with `window_days`. 12/36 (365/1095 day) sensitivity is filed and not frozen. Packet 1c compared 24-calendar-month semantics using UTC time preservation and month-end clamping; after the approved-snapshot replay, the human retained fixed 730-day runtime semantics on 2026-09-03.
...
**Historical packet context for item 4:** Its references to the 2026-09-09 C resolution and 294-file G census describe the then-current state on 2026-09-09 and are preserved as history. The active current release successors are recorded in the ledger above; G's current schema-closure successor remains unreviewed and non-authorizing.
4. **Operators B–G:** B is **supported**, non-default, and composition-gated. E is **experimental**, executable, off by default, and composition-gated. The default settings allowlist is `A`; only an explicit config override, such as `operator-e.override.toml`, may add E. `diverge --operator` may request A/B and cannot widen the allowlist; B requires explicit target and target-bound axiom proof. An application caller using an E-enabled composition must provide the same proof. An allowlisted composition may persist `operator=B` or `operator=E`, with scored cards preserving the corresponding `generating_operator`. C's current report-only resolution packet is `docs/reviews/nsqd-operator-c-evidence-resolution-2026-09-09/` (`8bec12564b873d1ae59a60ed0988b766e66909e7efa62a11a3fc027cfe06dd90`), a negative and insufficient result: both typed edges (`A_to_bridge` and `bridge_to_C`) and their composition are unsupported. The corrected packet records shuffled-control provenance as `unavailable_nonreproducible` because seed material, the candidate pool, the algorithm, and the permutation are not retained; the shuffle is not used as absence evidence. Successful zero-result queries mean only `not_observed_within_bounded_queries`; Semantic Scholar HTTP 429 responses mean `unavailable_from_service`; these results do not establish universal absence, noninteraction, novelty, or priority. Todo 6 technical review is confirmed by detached review summary SHA `643fc415cc2f6b266a7a8bf4c04fdf11e34ede3947dd3be090ad241d3a5f04d4`, seal SHA `43119d95b784fa587ae640815b8b71f6c96ccbc6f12d72ab1875edaff39971f2`, and reviewer session `ses_f76edce6dffe6gPRxa2M13UADd`; it confirms only the bounded negative report-only conclusion. Both typed edges and their composition remain unsupported, evidence remains insufficient, human acceptance was not requested, and schema admission, runtime authorization, and Todo 7 advancement remain false. The 2026-09-08 packet at `docs/reviews/nsqd-operator-c-evidence-2026-09-08/` (`7dc279651b4e70c2838d80b7e00cf7e9da7102ad93dcb87c462103f1c6cd4f43`) is an earlier independently reviewed C predecessor; the final C review packet is now the latest independently reviewed C evidence. D/F/G have executable report-only data contracts, but D remains blocked on C; F's evaluation-only readiness inventory has five observed and three eligible records, an unexecutable predeclared five-fold source-group protocol, missing held-out/stability/redundancy/residual/quality/confound evidence, and no human thresholds; G's current census is `docs/reviews/nsqd-operator-g-readiness-census-2026-09-09-c-review-correction/` (`2509690fecb8800bd99dcb726d31504876c6de37a1c66c5ced33562dcc1439e6`), complete over 294 structured files (`scope_snapshot_digest=9529d91e202fac4043b0faebec22d0460b284dfaf0168d042c5b30007c2c5405`; `readiness.json` SHA `b57f0146f7620f1d259d90e29efe413b3edde85d789bf4905c22aa994305dfbe`), with zero trusted approvals, zero trusted evidence artifacts, and zero qualifying records; all authority flags are false. It chains from the preserved predecessor at `docs/reviews/nsqd-operator-g-readiness-census-2026-09-09-c-review/` (`3a9070604f9e622bc3d2d56f666cc762ae31d85d7bad85c06e531e056c1efc87`), whose Todo 2 predecessor is `docs/reviews/nsqd-operator-g-readiness-census-2026-09-09-protocol/` (`47fa35d858a186a9b1c416cfe7c1003d177aa2c91d72ebd7c5e20d80810efcb8`). C, D, F, and G remain deferred. Executable `τ` alone remains insufficient for E (`docs/ablations/alg-operators.md`).
