...
**Supersedes**: `docs/development-plan-ns-qd.md` v1.6.54 wording (same file, revision)
...
| NSQD.OPERATOR.ACTIVATION_PLAN.v1 | CLI selection is A/B and cannot widen composition. C's external evidence report rejects all seven proposed bridges; candidate outputs remain empty and C, D, F, and G stay runtime-disabled. E is experimental, executable, and off by default (`operator-e-atypical-combination/1`); configuration may add E; B remains non-default; rarity-only generation is rejected; report-only JEPA artifacts remain ungenerated | Optional packet 5 | Data Contract, Behavior | LOCAL-NSQD-D | EV-N20 |
...
4. **Operators B–G:** B is **supported**, non-default, and composition-gated. E is **experimental**, executable, off by default, and composition-gated. The default settings allowlist is `A`; a config override may add `B`, `E`, or both. `diverge --operator` may request A/B and cannot widen the allowlist; B requires explicit target and target-bound axiom proof. An application caller using an E-enabled composition must provide the same proof. An allowlisted composition may persist `operator=B` or `operator=E`, with scored cards preserving the corresponding `generating_operator`. C, D, F, and G remain deferred. Executable `τ` alone remains insufficient for E (`docs/ablations/alg-operators.md`).
